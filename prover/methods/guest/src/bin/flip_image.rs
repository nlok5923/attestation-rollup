...
fn flip_image_horizontally(base64_string: &str) -> String {
    ...
    // Create a new ImageBuffer with the same dimensions
    let (width, height) = img.dimensions();
    let mut flipped_img = ImageBuffer::new(width, height);

    // Manually flip the image horizontally
    for y in 0..height {
        for x in 0..width {
            let pixel = img.get_pixel(x, y);
            flipped_img.put_pixel(width - x - 1, y, pixel);
        }
    }
    ...
    // Encode the flipped image as PNG and write to the buffer
    encoder.encode(
            &flipped_img,
            width,
            height,
    ...
    let base64_image = encode(&png_data);
    println!("Flipped Image Base64: {}", base64_image);
...
fn main() {
    ...
    let inverted_image_data = flip_image_horizontally(base64_str);
